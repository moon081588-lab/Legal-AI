...
| 검색 정확도 Recall@5 | 100% (16/16) | `python tests/evals/run_evals.py` (실패 시 exit 1) | ✅ 차단 |
...
| 조용한 데이터 손상 | `.sha256` 사이드카 | `python tools/ingest/rollback.py --verify` |
| 나쁜 데이터 배포 | 날짜별 스냅샷 + 롤백 명령 | `python tools/ingest/rollback.py` |
