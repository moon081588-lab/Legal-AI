...
1. **출처 없는 법률 콘텐츠 금지.** 사용자에게 보이는 모든 정보에는 확인 가능한 출처가 있어야 합니다. `data/content/*.json`에 항목을 추가할 때는 `sources: [{label, url}]`을 함께 넣고 `verified_on` 날짜를 갱신해 주세요(정부·법원·공공기관 출처 우선, https 필수). 테스트가 이를 강제합니다(`test_every_support_program_cites_a_source`, `test_reference_datasets_carry_sources`). 가이드·샘플 조문은 PR 설명에 근거(법령 조문, 판례 번호)를 명시해 주세요.
